...
const InvoiceTemplateSelector: React.FC<InvoiceTemplateSelectorProps> = ({
  ...
  const { templates, loading, error } = useTemplatesList();
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          <div>
            ...
            <p className="text-gray-500 mt-1">Choose a design for this invoice</p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="grid xs:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {templates.map((template, index) => (
                <TemplateCard
                  ...
                  onPreview={() => {}}
                  mode="select"
                  selected={String(currentTemplateId) === String(template.id)}
