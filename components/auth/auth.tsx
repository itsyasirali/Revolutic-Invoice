...
const LoginSignupForm: React.FC<LoginSignupFormProps> = ({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 sm:p-6 font-sans">
      <div className="w-full max-w-[800px] h-[600px] bg-white rounded-md shadow-2xl overflow-hidden flex flex-col md:flex-row relative animate-reveal">
        {/* Overlay Side */}
        <div
          className={`md:absolute md:top-0 md:bottom-0 w-full md:w-1/2 transition-all duration-700 ease-[cubic-bezier(0.645,0.045,0.355,1)] z-20 ${isSignup ? "md:translate-x-0" : "md:translate-x-full"}`}
        >
        ...
        <div className="w-full h-full relative flex flex-col md:flex-row">
          <div
            className={`w-full md:w-1/2 p-8 md:p-10 flex flex-col items-center justify-center space-y-4 md:space-y-6 bg-white transition-all duration-700 ease-[cubic-bezier(0.645,0.045,0.355,1)] ${isSignup ? "md:translate-x-full" : "md:translate-x-0"}`}
          >
            ...
            <p className="text-[11px] text-slate-400 font-medium">
              {isSignup
                ? "or register for Revolutic Invoice"
            ...
            <div className="w-full max-w-[300px] space-y-3">
              ...
              {!isSignup && (
                <div className="text-center pt-2">
                  <button className="text-xs text-slate-800 font-medium hover:underline">
                    Forgot password?
                  </button>
                </div>
              )}
